...
| | |
|---|---|
| **Ядро sing-box обновлено до 1.13** | Свежие исправления и совместимость. Главное — теперь Hysteria2 и NaïveProxy собираются **нативно** внутри sing-box, без отдельных обвязок. |
| **Нативный Hysteria2** | Полностью переехал на нативный outbound sing-box (auth, up/down, обфускация Salamander, порт-хоппинг) — быстрее, стабильнее, наш прежний внешний модуль удалён. |
| **NaïveProxy** | Новый протокол: маскирует трафик под обычный HTTP/2 браузера Chrome (движок Chromium cronet). Полезен там, где режут по сигнатурам (DPI). |
| **Прозрачный прокси (tproxy)** | Новый режим подключения: перехват на уровне ядра (TCP + UDP) без TUN. Только с root (нужен `CAP_NET_ADMIN`). |
| **Виджеты на рабочий стол** | Два виджета в тему приложения: переключатель и статус со скоростью `↓/↑`, переключением серверов `‹ ›` и кнопкой «Авто». Подключают/переключают прямо из сервиса — **без открытия приложения**; кнопка «Авто» теперь тоже работает в фоне и мгновенно даёт отклик. |
| **Защита сборки + дельта-обновления** | Предупреждение, если APK переподписан посторонним (перепакованная сборка с рекламой). Обновления скачиваются патчами ~1 МБ вместо полного APK. |
| **Обновлены ядра VK-TURN** | freeturn 1.6.0 и WDTT — свежая авторизация VK (режим `vkcalls`), капча и отпечатки. |
