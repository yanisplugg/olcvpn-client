...
> **Теперь и на Windows** — установщик и портативная версия одним `.exe`, x64 и нативный ARM64. То же приложение и те же движки, что на телефоне: подписки, профили маршрутизации, каскад, VK-TURN, olcRTC, DNSTT, Trust Tunnel.
...
| | |
|---|---|
| **Клиент для Windows** | Установщик (ru/en/zh/fa) и портативная версия **одним `.exe`**, x64 и нативный ARM64. Два режима: «Туннель» — свой сетевой адаптер, как на телефоне, и «Прокси» — локальный SOCKS5 + HTTP без прав администратора. |
| **Разделение по процессам** | Десктопный аналог раздельного туннелирования: выбранные программы идут мимо туннеля — или, наоборот, только они в туннель. |
| **Меню в трее + горячая клавиша** | Состояние, подключение, смена сервера, «Мой IP» и настройки прямо из трея. Глобальная комбинация переключает VPN из любого приложения. |
| **Обновлены ядра** | xray-core 26.7.28, sing-box 1.13.18, freeturn 2.1.1, olcRTC. QUIC (Hysteria2 / TUIC / DoQ / HTTP3) собран на всех платформах. |
| **FakeDNS больше не утекает мимо туннеля** | Синтетический адрес FakeDNS мог попасть под правило «приватные адреса → напрямую» — соединение уходило в обход туннеля, и приложения писали про недоверенный сертификат. Плюс таблица FakeDNS теперь переживает переподключение. |
| **Маршрутизация** | Списки правил (rule-set) качаются через туннель, а не напрямую; `asn:` работает и на десктопе. |
| **Дельта-обновления на десктопе** | Обновление тянет несколько мегабайт вместо всего установщика. |
