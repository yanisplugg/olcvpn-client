...
*VLESS · Reality · XHTTP поверх **Xray** и **sing-box** — а главное, фирменный транспорт **olcRTC**, который маскирует трафик под видеозвонок.*
...
В одном приложении — **два прокси-ядра** и фирменный транспорт **olcRTC**: заблокировали один способ — переключился и работаешь дальше.

> ⭐ **Главная изюминка — olcRTC.** Собственный транспорт проекта: гоняет трафик через настоящие сервисы видеозвонков (Jazz, Telemost, WB Stream, Jitsi). Для DPI это не прокси, а обычный звонок — поэтому работает там, где обычные протоколы уже режут.
...
| ⭐ **olcRTC** *(изюминка)* | Фирменный транспорт ([olcRTC](https://github.com/openlibrecommunity/olcrtc)) — трафик идёт через реальные сервисы видеозвонков, для DPI это обычный звонок, а не прокси. |
...
olcrtc/     olcRTC — фирменный транспорт      (вендорено)
