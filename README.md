...
*VLESS · Reality · XHTTP поверх **Xray** и **sing-box** — а главное, универсальность: даже поддержка **olcRTC**, маскирующего трафик под видеозвонок.*
...
В одном приложении — **два прокси-ядра** плюс поддержка транспорта **olcRTC**: заблокировали один способ — переключился и работаешь дальше.

> ⭐ **Главная особенность — универсальность.** Xray и sing-box, все ходовые протоколы и транспорты, импорт чего угодно — и вдобавок поддержка **olcRTC** ([openlibrecommunity/olcrtc](https://github.com/openlibrecommunity/olcrtc)), который гоняет трафик через настоящие сервисы видеозвонков (Jazz, Telemost, WB Stream, Jitsi) и для DPI выглядит как обычный звонок. Один способ зарезали — рядом ещё несколько.
...
| 🎭 **Поддержка olcRTC** | Транспорт [olcRTC](https://github.com/openlibrecommunity/olcrtc) (от openlibrecommunity) — трафик идёт через реальные сервисы видеозвонков, для DPI это обычный звонок, а не прокси. |
...
olcrtc/     olcRTC — транспорт-маскировка     (сторонний, вендорено)
