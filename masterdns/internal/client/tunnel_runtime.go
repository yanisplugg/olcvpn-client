...
func dialUDPResolver(resolverLabel string) (*net.UDPConn, error) {
	...
	// LOCAL PATCH (olcvpn-client): route through netutil so Android can VpnService.protect() the
	// socket. Without a protector installed this is exactly net.DialUDP.
	return netutil.DialUDPProtected(addr)
}
