...
func (c *SplitHTTPConfig) Build() (proto.Message, error) {
	...
	// LOCAL PATCH: honour the pre-26.7.28 session* spellings when the sessionID* ones are absent.
	if c.SessionIDPlacement == "" {
		c.SessionIDPlacement = c.LegacySessionPlacement
	}
	if c.SessionIDKey == "" {
		c.SessionIDKey = c.LegacySessionKey
	}
