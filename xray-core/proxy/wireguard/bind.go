...
type netBind struct {
	...
	// closeMu guards closedCh/closed: Close can race with itself (handler
	// shutdown vs an in-flight processWireGuard switching dialers) and a bare
	// close(closedCh) then panics with "close of closed channel", taking down
	// the whole process on mobile. Closing is idempotent per Open.
	closeMu  sync.Mutex
...
func (bind *netBind) Close() error {
	...
	bind.closeMu.Lock()
	defer bind.closeMu.Unlock()
	if bind.closedCh != nil && !bind.closed {
		close(bind.closedCh)
	}
	bind.closed = true
	return nil
