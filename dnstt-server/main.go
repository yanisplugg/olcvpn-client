...
func main() {
	...
	var udpAddr string
	// olcvpn additions: a -domain flag (so the installer can pass the tunnel domain without a
	// positional arg) and a built-in SOCKS5 upstream. The mobile dnstt client pipes raw SOCKS5
	// through each tunnel stream, so the upstream MUST speak SOCKS5. With -socks-port set, the
	// server runs its own no-auth SOCKS5 listener on 127.0.0.1:<port> and uses it as the upstream,
	// making the VPS a one-binary internet exit (no separate proxy to install).
  ...
  %[1]s -udp ADDR -privkey-file PRIVKEYFILE -domain DOMAIN -socks-port 8000
...
Example:
  ...
  %[1]s -udp :5300 -privkey-file server.key -domain t.example.com -socks-port 8000
...
`, os.Args[0])
	...
	flag.StringVar(&domainFlag, "domain", "", "tunnel domain (alternative to the positional DOMAIN argument)")
	flag.IntVar(&socksPort, "socks-port", 0, "run a built-in SOCKS5 upstream on 127.0.0.1:<port> and use it as UPSTREAMADDR")
	flag.Parse()
	...
	if genKey {
		...
		// Ordinary server mode. The tunnel domain comes from -domain or the first positional arg;
		// the upstream comes from -socks-port (built-in SOCKS5) or the next positional arg.
		domainStr := domainFlag
		args := flag.Args()
		if domainStr == "" {
			if len(args) == 0 {
				fmt.Fprintf(os.Stderr, "a tunnel domain is required (-domain or positional DOMAIN)\n")
				flag.Usage()
				os.Exit(1)
			}
			domainStr = args[0]
		...
		var upstream string
		if socksPort != 0 {
			upstream = fmt.Sprintf("127.0.0.1:%d", socksPort)
			if err := startSocks5(upstream); err != nil {
				fmt.Fprintf(os.Stderr, "starting built-in SOCKS5 upstream: %v\n", err)
				os.Exit(1)
			}
			log.Printf("built-in SOCKS5 upstream listening on %s", upstream)
		} else {
			if len(args) != 1 {
				fmt.Fprintf(os.Stderr, "an upstream address is required (UPSTREAMADDR or -socks-port)\n")
				flag.Usage()
				os.Exit(1)
			}
			upstream = args[0]
